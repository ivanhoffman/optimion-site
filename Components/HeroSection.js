...
import CalDotComModal from "@/Components/CalDotComModal"; // ⟵ Cal.com modal
...
function HeroVisual() {
  ...
  const chips = CHIPS.map((c, i) => {
    const baseR =
      c.rFromRing !== undefined ? R_RING + c.rFromRing : c.radius ?? R_DEFAULT;
        ...
        {chips.map((p) => (
          <motion.span
            ...
            className="
              ...
              border border-neutral-800 bg-black/60
              text-gray-200 text-xs md:text-sm
              px-3 py-1 leading-[1.05]
              backdrop-blur-[2px]
              whitespace-nowrap
          ...
          >
            ...
            <span className="relative top-[0.5px]">{p.label}</span>
          </motion.span>
        ))}
      </div>
    </div>
